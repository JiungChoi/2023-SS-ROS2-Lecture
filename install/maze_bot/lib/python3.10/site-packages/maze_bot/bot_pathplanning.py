...
class bot_pathplanner():
    ...
    def find_path_nd_display(self,graph,start,end,maze,method = "DFS"):
        ...
        if (method =="dijisktra"):            
            if (self.dijisktra.shortest_path_overlayed==np.zeros_like(self.dijisktra.shortest_path_overlayed)):
                self.draw_path_on_maze(maze,pathpts_to_display,method)
            else:
                if config.debug and config.debug_pathplanning:
                    print(self.dijisktra.shortest_path_overlayed)
            ...
            if (self.astar.shortest_path_overlayed==np.zeros_like(self.dijisktra.shortest_path_overlayed)):
                self.draw_path_on_maze(maze,pathpts_to_display,method)
            else:
                if config.debug and config.debug_pathplanning:
